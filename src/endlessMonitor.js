...
module.exports = async (client) => {
  // timer for 30 minutes, calls webScraperHandler when timer is up
  const interval = setInterval(scanSiteAndMembers, 1800000 / 2, client) // 30 / 2 minutes timer
}

const scanSiteAndMembers = async (client) => {
  scanAllMembers(client)
  webScraperHandler(client)
}

const scanAllMembers = async (client) => {
  try {
    ...
    for (const member of members) {
      if (member[1].roles.cache.has(staffRole)) {
        ...
        if (member[1].roles.cache.has(translatorRole)) {
